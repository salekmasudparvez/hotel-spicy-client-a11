...
const RoomCard = ({ room }) => {
    const { RoomImages, RoomDescription, PricePerNight, Features, Availability } = room
    console.log(room);
    return (
        <div className="flex flex-col w-fit border items-center shadow-2xl justify-center rounded-2xl max-w-sm mx-auto">
            <div className="relative bg-transparent object-cover rounded-2xl z-0]"><img className=' w-[250px] md:w-[300px] rounded-2xl object-cover h-64' src={RoomImages} />
            {Availability?
            <div className=" absolute top-0 text-white badge badge-success gap-2">
                Available
            </div>:
            <div className="absolute top-0 text-white badge badge-error gap-2">
            Unavailable
          </div>
            }
            </div>
