...
const RoomDetails = () => {
    ...
    const handleMyBooking = (event) => {
        ...
        const getRoomData = {
            ...
            bookingID: _id,
            imageUrl:RoomImages,
            bookingDate:event.target.date.value
        }
        axios.post('https://hotel-server-kappa.vercel.app/create-payment', getRoomData)
            .then(res => {
    ...
    const { isLoading: reviewLoading, data: reviews } = useQuery({
        ...
        queryFn: async () => {
            const response = await axios.get(`https://hotel-server-kappa.vercel.app/review?id=${_id}`)
            const data = response.data
            return data
        }
    })
